...
Options considered: (a) ignore its structure and import from zero; (b) adopt its store and lockfile as SkillVault's own; (c) keep SkillVault's own vault but consume the installer lockfile as provenance evidence at import.

## Decision

Option (c). SkillVault maintains its own vault (`~/.skillvault/vault/`). During import, `.skill-lock.json` metadata is mapped into the provenance model as **Declared** evidence, upgradeable to **Verified** after re-verification against the declared repository. The npm registry is not a source type in the MVP.

## Consequences

- Importing an existing `npx skills` setup yields immediate provenance for every skill instead of starting from Unknown.
- SkillVault does not read or write `~/.agents` after import; two tools writing one store cannot coexist with transactional guarantees. Post-import, SkillVault expects to be the single manager.
