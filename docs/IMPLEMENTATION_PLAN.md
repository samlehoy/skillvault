...
Progress: Milestone 0 and Milestone 1 are complete (M1 finished 2026-07-26: schemas, skill IDs and collisions, `SKILL.md` validation, override/disable resolution, source descriptors, ownership/actual/plan/operation models, cross-project fixtures, and determinism tests — 92 passing core tests). Milestone 2 is in progress (as of 2026-07-26): junction primitives, content hashing, verified backups, OpenCode read-only discovery (field-tested against a real installation), vault ingestion with immutable hash-keyed revisions, link/unlink planners, and the transaction executor with lock, staleness gate, and automatic rollback are done. The headless `doctor`, the Ink TUI, the real TuiCore facade, and the build/CLI wiring are also done, and a scripted acceptance run against a real Windows machine passed on 2026-07-26: a disposable OpenCode skill was managed end to end through the TUI (cancel proven mutation-free; apply produced a verified vault junction and backup; the machine was restored to its original state afterwards). The interactive Windows Terminal check passed by owner screenshot, and the TUI was redesigned to the skill-first flow in `docs/TUI_FLOW.md` (aggregated inventory, action panel, conflict gate, search/filters/help). Remaining in M2: confirming OpenCode loads a junction-linked skill in a live session, and the first public 0.x release (blocked on npm login). M3 has started: the Antigravity discovery adapter (both variants) and the shared adapter contract test suite are done and wired into the facade as visibility-only targets; remaining in M3: sync/uninstall parity and a live verification that Antigravity's loader follows junctions (until then Antigravity is not offered as a creatable link target). M4 core landed 2026-07-27: the system-git runner (non-interactive, credentials fully delegated), the bare-mirror cache with commit materialization via transient worktrees, `resolveGitSource` (ref → commit SHA + normalized content hash, explicit subdirectory support), `syncLockedGitSource` (installs exactly the locked commit, offline from cache, fails closed on hash mismatch or missing commits), the read-only `checkGitUpdate` (old/new commit + changed files under the source scope), and the project manifest/lockfile file workflow with a deterministic source-control-friendly serializer — all covered by integration tests against local temporary repositories (moved branches, unavailable remotes, fresh-environment reproducibility, no credential persistence). Remaining in M4: wiring git sources into the sync/TUI flow once M5's import model lands. M5 started 2026-07-27 with three slices landed: installer lockfile parsers for the vercel-labs `.skill-lock.json` (v3) and Antigravity `skills-lock.json` (v1) producing Declared bundle labels (fail-closed on unknown versions, warning-not-failure on malformed third-party entries), skill fingerprinting with deterministic exact-duplicate/conflict/likely-duplicate classification over read-only discovery, and the TUI bundle grouping from TUI_FLOW.md (`g` toggle, bundle sections with `(unknown source)` fallback, bundle membership in the detail and action panels). Further M5 work landed the same day: the installer-junction takeover fixture (which exposed and fixed an ingest bug: junction sources are now staged by dereferenced content), the persistent transaction journal (debt cleared, see above), the recovery surface (TUI banner plus a failing `doctor` check listing interrupted plans and preserved backups), and the first-run hint shown while nothing is managed yet. Remaining in M5: a richer guided import flow (batch import candidates surfaced as a dedicated view) is deferred until batch selection mechanics arrive in M7; single-skill import via the action panel is complete. M6 core landed 2026-07-27: the provenance confidence model (unknown/inferred/declared/user-verified/verified with evidence chains; no name-similarity input exists, and `verified` is reachable only through a real re-verification result), editable provenance (persistent user assertions, `s` in the action panel, user assertions outrank installer declarations and show their confidence), the read-only audit engine with stable finding IDs/severities/remediations plus `skillvault audit [--json]`, and the read-only MCP + Plugins inventory tabs (config layouts verified on the reference machine; secret values never enter the view model, URL query strings stripped, same-name-different-config findings). Remaining in M6: wiring re-verification against the Git cache (upgrading Declared → Verified at runtime) and freshness display, source-status inventory filters, and the audit categories that need manifest/lock integration (outdated revisions, manifest-lock mismatch, unused entries, scope conflicts) — those follow the M4 sync wiring. M3 sync parity landed 2026-07-27 behind a capability gate: the facade derives per-location installation IDs (no longer hardcoded to OpenCode), offers both Antigravity variants as creatable link targets, and a facade test proves one vault entry synchronizes to OpenCode plus both Antigravity variants in a single plan with all junctions resolving to the same revision — but the gate (`ANTIGRAVITY_JUNCTION_CONSUMPTION_VERIFIED`) stays off in production until the live probe check passes. Disposable junction probes were installed into the OpenCode and both Antigravity skill directories on the reference machine; flipping the constant (plus recording the evidence in M0_VERIFIED_FACTS.md) closes M3 once the owner confirms both IDEs read the probe.
...
- ~~Transaction records are returned in memory but not yet persisted under `~/.skillvault/`~~ — cleared 2026-07-27: every mutation now writes a journal entry under `~/.skillvault/state/transactions/` (`in-progress` → `applied`/`rolled-back`/`rollback-failed`), so a crash mid-run is detectable; the TUI shows a recovery banner and `doctor` fails with per-plan detail when interrupted entries exist.
